...
int kmNet_init(char* ip, char* port, char* mac)
{
	...
	if (WSAStartup(MAKEWORD(1, 1), &wsaData) != 0) 		return err_creat_socket;
	if (LOBYTE(wsaData.wVersion) != 1 || HIBYTE(wsaData.wVersion) != 1) {
		WSACleanup(); sockClientfd = 0;
	...
	addrSrv.sin_port = htons((u_short)atoi(port));//端口UUID[1]>>16高16位
	tx.head.mac = to_hex(mac, 4);		 //盒子的mac 固定 UUID[1]
	tx.head.rand = rand();				 //随机值。后续可用于网络数据包加密。避免特征。先预留
	tx.head.indexpts = 0;				 //指令统计值
	tx.head.cmd = 0xaf3c2828;			 //指令
	...
	if (recvfrom(sockClientfd, (char*)&rx, 1024, 0, (struct sockaddr*)&addrSrv, &clen) < 0)
