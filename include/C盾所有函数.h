...
extern "C" {
    ...
    //后台更新文件,参数 _cs_function 是回调函数 原形void WINAPI DownloadProgress(_CS_GetCallbackProgress ScheduleStructure); 用途下载文件时设置下载进度条
    ...
    //充值
      void _Recharge(const char* _serialnumber, const char* _user, char* buf/*=NULL*/);
    //修改密码
    ...
    //自定获取机器码
     ...
     //获取公告列表 返回值 返回公告列表个数 参数1 获取类型 ，参数2 分割符，参数3返回的公告列表  参数4 返回长度 公告列表=｛名称 参数2 分割符 名称 参数2 分割符 名称｝ 获取类型==1 返回 _GetAnnouncement 相关表数据 获取类型==2 返回 _GetAnnouncementData 相关表数据
